...
\end{equation}
where $|B_d|$ is the number of group elements.
The averaged filters are then reduced to an ``eigen-set'' of orthogonal filters by the singular value decomposition.
In detail we started this averaging with a set of independent $k'$-$p'$-tensor convolution filters that span the space of all possible geometric convolution filters at side length $M$ ($M=2\,m+1$) and dimension $d$.
There are $M^d\,d^{k'}$ independent filters in this set.
...
There are no invariant pseudoscalar filters available at $(d,m) = (2,1)$.
Three of the $2$-$0$-tensors are duplicates of the $0$-$0$-tensor filters but multiplied by the identity tensor.\label{fig:filters23}}
  \end{mdframed}
\end{figure}
\begin{figure}[tp]
  \begin{mdframed}
